...
{#if rejectedApprovals.length > 0}
  <div class="rounded-md border-l-4 border-l-amber-400 bg-amber-50/50 p-4">
    <div class="flex items-start">
      <div class="shrink-0">
        <svg class="size-5 text-amber-500" viewBox="0 0 20 20" fill="currentColor">
          <path
            fill-rule="evenodd"
        ...
        <div class="mt-2 text-sm text-amber-700">
          {#each rejectedApprovals as { id, respondedBy, role, studyProgram, comment } (id)}
            ...
            <div class="rounded border border-amber-200 bg-white p-3">
              {#if comment?.trim()}
                <p class="text-sm whitespace-pre-wrap text-gray-700">{comment}</p>
              {:else}
                <p class="text-sm text-gray-500 italic">Kein Kommentar angegeben</p>
              {/if}
            </div>
          {/each}
        </div>
