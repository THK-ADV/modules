<script lang="ts">
  ...
  import { ModulePreviewPlaceholder } from '$lib/components/module-detail'
  ...
  let selectedModule = $derived(
    moduleDrafts.find(({ module }) => module.id === selectedModuleId) ?? null
  )
  ...
  const columns: ColumnDef<ModuleDraft>[] = [
    {
      ...
      cell: ({ row }) =>
        renderComponent(ModuleDraftTitleCell, {
          moduleDraft: row.original,
          onPreview: (moduleDraft: ModuleDraft) => (selectedModuleId = moduleDraft.module.id)
        }),
      filterFn: (row, _, filterValue) => {
        if (!filterValue) return true
        const query = filterValue.toString().toLowerCase()
        const title = (row.original.moduleDraft?.title ?? row.original.module.title).toLowerCase()
        return (
          title.includes(query) ||
          (row.original.moduleDraft?.abbreviation ?? row.original.module.abbreviation)
            .toLowerCase()
            .includes(query)
        )
      }
    },
    {
      ...
      header: 'Status',
      cell: ({ row }) =>
        renderComponent(ModuleDraftStatusCell, { state: row.original.moduleDraftState }),
      enableColumnFilter: false
    },
    {
      id: 'actions',
      cell: ({ row }) =>
        renderComponent(ModuleDraftTableActions, {
          ...
          onActionComplete: () => (selectedModuleId = null)
        })
    }
  ]
...
<div class="flex min-h-0 flex-1">
  <div class="min-w-0 flex-1 space-y-8 lg:pr-6">
    ...
    {#if moduleDrafts.length > 0}
      <ModuleDraftTable {moduleDrafts} {columns} {hasAdditionalModules} {selectedModuleId} />
    {:else}
      <Empty.Root class="border-border/70 bg-muted/30 min-h-80 border">
        <Empty.Header class="gap-3">
          ...
          <Empty.Description class="max-w-sm">
            Sie werden derzeit in keinem Modul als Modulverantwortliche*r geführt und haben keine
            über Rollen zugeteilten Module.
          </Empty.Description>
        </Empty.Header>
      </Empty.Root>
    {/if}
  ...
  <aside
    aria-label="Modulvorschau"
    class="sticky top-6 hidden h-[calc(100vh-7rem)] shrink-0 overflow-hidden border-l transition-[width] duration-200 lg:block {selectedModule
      ? 'w-[min(42vw,48rem)]'
      : 'w-0 border-l-0'}"
  >
    {#if selectedModule}
      <div class="h-full w-[min(42vw,48rem)]">
        <ModulePreviewPlaceholder
          id={selectedModule.module.id}
          moduleDraftState={selectedModule.moduleDraftState}
          onClose={() => (selectedModuleId = null)}
        />
      </div>
    {/if}
  </aside>
